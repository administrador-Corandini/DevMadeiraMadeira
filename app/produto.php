...
class produto extends Model
{
    ...
    public function cadastraProdutoSatisfacao($cliente_id,$id_produto ,$nome_produto,$data_entregue_produto,$data_prometido_pedido,$id_pedido_marketplace,$marketplace_id,$carteira_id){
        ...
        $buscaProduto = produto::where('id_produto',$id_produto)->get();//Faz a busca se ja esta inserido
        
        if (count($buscaProduto) == 0) {
    ...
    public function cadastraProdutoTransporte($cliente_id ,$id_produto,$nome_produto,$data_SAC,$link,$loja_venda,$carteira_id,$id_reclamacao){

        $buscaProduto = produto::where('id_reclamacao',$id_reclamacao)->get();//Faz a busca se ja esta inserido
